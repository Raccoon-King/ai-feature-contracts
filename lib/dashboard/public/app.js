...
function renderContractList() {
  ...
  filtered.forEach((contract, index) => {
    ...
    cardContent.addEventListener('click', () => loadContract(contract.id));
...
function renderRulesEditor() {
  ...
  if (els.rulesEditorStatus) {
    els.rulesEditorStatus.textContent = state.rulesEditorModified ? 'Modified' : '';
    els.rulesEditorStatus.classList.toggle('modified', state.rulesEditorModified);
  }

  // Update button states
  if (els.rulesSaveDraftBtn) {
    els.rulesSaveDraftBtn.disabled = !state.rulesEditorModified;
  }

  if (els.rulesApplyBtn) {
    els.rulesApplyBtn.disabled = !state.rulesEditorModified;
  }

  if (els.rulesDiscardBtn) {
    els.rulesDiscardBtn.disabled = !state.rulesEditorModified;
  }
...
async function loadRuleDetail(scope, value) {
  ...
  try {
    ...
    // Load content into editor for local rulesets
    if (scope === 'local' && state.ruleDetail?.content) {
      loadRuleIntoEditor(
