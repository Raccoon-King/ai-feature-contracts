...
// Contract ID pattern - must match validation regex
const CONTRACT_ID_PATTERN = /^[A-Z][A-Z0-9]*(?:-[A-Z0-9]+)*-\d+$/;

/**
 * Sanitize contract ID to prevent path traversal
...
function sanitizeContractId(id) {
  ...
  if (!CONTRACT_ID_PATTERN.test(basename)) return null;
  // Verify no path traversal attempted
  if (basename !== id) return null;
  return basename;
}
