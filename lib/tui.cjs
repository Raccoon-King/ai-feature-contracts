...
function createMenu(options = {}) {
  ...
  const render = () => {
    ...
    items.forEach((item, index) => {
      ...
      const prefix = isDisabled ? ' ' : (isSelected ? `${GREEN}>${RESET}` : ' ');
      const style = isDisabled ? DIM : (isSelected ? BOLD : '');
      const label = item.label || item;
      const desc = (item.description && !isDisabled) ? `${DIM} - ${item.description}${RESET}` : '';
  ...
  // Find next valid (non-disabled) index in direction
  const findNextValid = (currentIndex, direction) => {
    let next = currentIndex + direction;
    while (next >= 0 && next < items.length && items[next].disabled) {
      next += direction;
    }
    return (next >= 0 && next < items.length) ? next : currentIndex;
  };

  const handleKey = (key) => {
    if (key === '\u001B[A') {
      selectedIndex = findNextValid(selectedIndex, -1);
      render();
    } else if (key === '\u001B[B') {
      selectedIndex = findNextValid(selectedIndex, 1);
      render();
...
function createTUI(context) {
  ...
  function mainMenu() {
    ...
    const menu = createMenu({
      ...
      items: [
        { label: 'View Contracts', description: !setupComplete ? 'Locked until setup completes' : 'Browse and manage your feature contracts', action: 'list' },
        { label: 'Setup & Onboarding', description: setupComplete ? 'Configure project settings, context, and rulesets' : 'Required first step - configure Grabby for this project', action: 'setup' },
        { label: 'Start New Work', description: !setupComplete ? 'Locked until setup completes' : ((hasBootstrap && !bootstrapComplete) ? 'Locked until bootstrap story completes' : 'Create a new contract or continue existing work'), action: 'create' },
        { label: 'Validate All', description: !setupComplete ? 'Locked until setup completes' : ((hasBootstrap && !bootstrapComplete) ? 'Locked until bootstrap story completes' : 'Run validation checks on all contracts'), action: 'validate' },
        { label: 'Metrics & Health', description: !setupComplete ? 'Locked until setup completes' : ((hasBootstrap && !bootstrapComplete) ? 'Locked until bootstrap story completes' : 'View project health and contract statistics'), action: 'metrics' },
        { label: 'Watch Mode', description: !setupComplete ? 'Locked until setup completes' : ((hasBootstrap && !bootstrapComplete) ? 'Locked until bootstrap story completes' : 'Auto-validate contracts on file changes'), action: 'watch' },
        { label: 'CI/CD Automation', description: !setupComplete ? 'Locked until setup completes' : ((hasBootstrap && !bootstrapComplete) ? 'Locked until bootstrap story completes' : 'Configure continuous integration hooks'), action: 'cicd' },
        { label: 'Manage Plugins', description: !setupComplete ? 'Locked until setup completes' : ((hasBootstrap && !bootstrapComplete) ? 'Locked until bootstrap story completes' : 'View and configure installed plugins'), action: 'plugins' },
        { label: 'Settings', description: !setupComplete ? 'Locked until setup completes' : ((hasBootstrap && !bootstrapComplete) ? 'Locked until bootstrap story completes' : 'Customize Grabby behavior and preferences'), action: 'settings' },
        { label: 'Rulesets', description: !setupComplete ? 'Locked until setup completes' : ((hasBootstrap && !bootstrapComplete) ? 'Locked until bootstrap story completes' : 'Import or author repository rulesets'), action: 'rulesets' },
        { label: 'Open Dashboard', description: !setupComplete ? 'Locked until setup completes' : ((hasBootstrap && !bootstrapComplete) ? 'Locked until bootstrap story completes' : 'Launch the local web UI and browser view'), action: 'dashboard' },
        { label: 'Exit', description: 'Close Grabby menu', action: 'exit' },
      ],
  ...
  function rulesetMenu() {
    ...
    const menu = createMenu({
      ...
      items: [
        ...
        { label: 'Generate Shared Rules', description: 'Create protected shared ruleset from guidance (protected)', action: 'generate-shared' },
        { label: 'Update Shared Rules', description: 'Update existing protected shared ruleset', action: 'update-shared' },
        { label: 'List Shared Rules', description: 'View protected shared rulesets', action: 'list-shared' },
        { label: 'Back', description: 'Return to the main menu', action: 'back' },
      ],
      onSelect: (item) => {
        process.stdout.write(CLEAR_SCREEN);
        ...
        // Shared rules authoring actions
        if (item.action === 'generate-shared') {
          const { generateSharedRules } = require('./rules-authoring.cjs');
          console.log(`${CYAN}Generating shared rules from repository guidance...${RESET}\n`);
          Promise.resolve(generateSharedRules({ cwd, logger: console }))
            .then((result) => {
              console.log(`\n${GREEN}✓ Shared ruleset generated: ${result.relativePath}${RESET}`);
              console.log(`${DIM}This location is write-protected from normal operations.${RESET}`);
              waitForKey(mainMenu);
            })
            .catch((error) => {
              console.log(`${RED}Generation failed: ${error.message}${RESET}`);
              waitForKey(mainMenu);
            });
          return;
        }

        if (item.action === 'update-shared') {
          const { updateSharedRules } = require('./rules-authoring.cjs');
          console.log(`${CYAN}Updating shared rules...${RESET}\n`);
          Promise.resolve(updateSharedRules({ cwd, logger: console }))
            .then((result) => {
              console.log(`\n${GREEN}✓ Shared ruleset updated: ${result.relativePath}${RESET}`);
              waitForKey(mainMenu);
            })
            .catch((error) => {
        ...
        if (item.action === 'list-shared') {
          const { listSharedRulesets, getProtectedRulesPath } = require('./rules-authoring.cjs');
          const path = require('path');
          const protectedPath = getProtectedRulesPath(cwd);
          const rulesets = listSharedRulesets(cwd);
          console.log(`${BOLD}Shared Rulesets${RESET}`);
          console.log(`Protected path: ${CYAN}${path.relative(cwd, protectedPath) || protectedPath}${RESET}\n`);
          if (rulesets.length === 0) {
            console.log(`${DIM}No shared rulesets found.${RESET}`);
            console.log(`${DIM}Use "Generate Shared Rules" to create one.${RESET}`);
          } else {
            rulesets.forEach((rs, i) => {
              console.log(`${CYAN}${i + 1}.${RESET} ${rs.name}`);
              console.log(`   Path: ${rs.relativePath}`);
              console.log(`   Modified: ${DIM}${rs.modified.toLocaleString()}${RESET}\n`);
            });
          }
          waitForKey(mainMenu);
          return;
        ...
        // Original ruleset wizard actions
        const { runRulesetWizard } = require('./ruleset-builder.cjs');
  ...
  function dashboardMenu() {
    process.stdout.write(CLEAR_SCREEN);
    console.log(`${CYAN}Starting Grabby dashboard...${RESET}\n`);

    const { runDashboard } = require('./dashboard/index.cjs');
    Promise.resolve(runDashboard({ cwd, logger: console }))
      .then(async ({ url, stop }) => {
        console.log(`${GREEN}Dashboard ready at ${url}${RESET}`);
        console.log(`${DIM}Press any key to stop the dashboard server and return to the menu.${RESET}`);
        if (typeof process.stdin.setRawMode === 'function') {
          process.stdin.setRawMode(true);
        }
        process.stdin.resume();
      ...
      .catch((error) => {
        console.log(`${RED}Failed to start dashboard: ${error.message}${RESET}`);
        waitForKey(mainMenu);
      });
  }
