...
const { selectPersonaForStage, deriveWorkflowRoles } = require('./personas.cjs');
const { readRoutingMemory, recordRoutingOutcome } = require('./routing-memory.cjs');
...
const { formatContractSummaryBox } = require('./tui.cjs');
...
/**
 * Build pre-op summary box from contract content
 */
function buildPreOpSummary(contractContent, planContent = null) {
  const title = contractContent.match(/^# FC:\s+(.+)$/m)?.[1] || 'Contract Summary';
  const overview = contractContent.match(/## Context[\s\S]*?\n([^\n#]+)/)?.[1]?.trim() || '';

  // Extract requirements from Done When section
  const doneWhenMatch = contractContent.match(/## Done When[\s\S]*?(?=## |$)/);
  const requirements = [];
  if (doneWhenMatch) {
    const items = doneWhenMatch[0].match(/- \[[ x]\]\s*(.+)/gi) || [];
    items.slice(0, 4).forEach(item => {
      requirements.push(item.replace(/- \[[ x]\]\s*/i, ''));
    });
  }

  // Extract files from Allowed Directories or plan
  const files = [];
  const allowedDirs = contractContent.match(/## Allowed Directories[\s\S]*?(?=## |$)/);
  if (allowedDirs) {
    const dirs = allowedDirs[0].match(/- `([^`]+)`/g) || [];
    dirs.slice(0, 3).forEach(d => {
      files.push({ file: d.replace(/- `|`/g, ''), changes: 'allowed directory' });
    });
  }

  // Parse plan phases if available
  const phases = [];
  if (planContent) {
    try {
      ...
      if (plan.files) {
        ...
        plan.files.forEach(f => {
          const action = f.action || 'modify';
          if (!byAction[action]) byAction[action] = [];
          byAction[action].push(f.path);
        });
        Object.entries(byAction).forEach(([action, paths]) => {
          phases.push({
            name: `${action.charAt(0).toUpperCase() + action.slice(1)} Files`,
            items: paths.slice(0, 5)
          });
        });
      }
  ...
  // Extract verification from Done When
  const verification = requirements.slice(0, 4);

  return formatContractSummaryBox({
    title,
    overview,
    requirements,
    phases,
...
/**
 * Save pre-op summary to .grabby for pre-commit hook
 */
function savePreOpSummary(grabbyDir, contractId, summary) {
  const preOpDir = path.join(grabbyDir, 'pre-ops');
  ensureDir(preOpDir);
  const preOpPath = path.join(preOpDir, `${contractId}.preop.txt`);
  fs.writeFileSync(preOpPath, summary);
  return preOpPath;
}
...
const shouldShowRoutingDebug = () => process.env.GRABBY_DEBUG === '1';
...
async function runGeneratePlanWorkflow(rl, agentData, contractFile) {
  ...
  const plan = {
    ...
    phase: 'plan',
    timestamp: timestamp(),
    context: contexts.length > 0 ? contexts : ['SYSTEM:architecture@v1', 'SYSTEM:standards@v1', 'SYSTEM:workflow@v1'],
...
async function collectTicketIntake(rl, request, input = {}, nonInteractive = false, options = {}) {
  ...
  if (questions.length > 0) {
    if (nonInteractive) {
      ...
      const guidance = [];
      if (validation.missingFields.includes('ticketId')) guidance.push('--ticket-id');
      if (validation.missingFields.includes('who')) guidance.push('--who');
      if (validation.missingFields.includes('what')) guidance.push('--what');
      if (validation.missingFields.includes('why')) guidance.push('--why');
      if (validation.missingFields.includes('dod')) guidance.push('--dod');
      const guidanceText = guidance.length > 0 ? guidance.join('/') : 'structured ticket text';
      throw new Error(`Ticket intake incomplete. Provide ${validation.missingFields.join(', ')} via structured ticket text or ${guidanceText}.`);
    }
...
async function runTaskBreakdownWorkflow(rl, seedRequest = '', options = {}) {
  ...
  const requireTicketId = Boolean(input.ticketId);
  ...
  console.log(`${c.dim(`Routing reason: ${persona.reason}`)}`);
  if (persona.blockedTransitions && persona.blockedTransitions.length > 0) {
    persona.blockedTransitions.forEach((entry) => {
      console.log(c.warn(`Blocked: ${entry.stage} - ${entry.reason}`));
    });
  }
  if (orchestrate) {
  ...
  if (shouldShowRoutingDebug()) {
    console.log(c.dim(`Routing decision: ${JSON.stringify(workflowRoles.decision || {}, null, 2)}`));
  }
  ...
  if (orchestrate) {
    const logRoutingCheckpoint = (checkpoint, checkpointContext) => {
      ...
      console.log(c.dim(`Routing checkpoint (${checkpoint}): ${roles.primary.agentName} - ${roles.primary.reason}`));
      if (shouldShowRoutingDebug()) {
        console.log(c.dim(`Checkpoint decision: ${JSON.stringify(roles.decision || {}, null, 2)}`));
      }
      return roles;
    };
    ...
    logRoutingCheckpoint('validation', {
      hasContract: true,
    ...
    logRoutingCheckpoint('execution', {
      hasContract: true,
      contractStatus: 'approved',
      hasPlan: true,
      planApproved: true,
    ...
    logRoutingCheckpoint('verification', {
      hasContract: true,
      contractStatus: 'approved',
      hasPlan: true,
      planApproved: true,
  ...
  // Build and display pre-op summary
  const preOpContractContent = fs.readFileSync(contractPath, 'utf8');
  const preOpPlanPath = path.join(CONTRACTS_DIR, fileName.replace('.fc.md', '.plan.yaml'));
  const preOpPlanContent = fs.existsSync(preOpPlanPath) ? fs.readFileSync(preOpPlanPath, 'utf8') : null;
  const preOpSummary = buildPreOpSummary(preOpContractContent, preOpPlanContent);

  console.log('\n' + c.heading('PRE-OP SUMMARY'));
  console.log(preOpSummary);

  // Save pre-op for pre-commit hook
  const preOpGrabbyDir = path.join(cwd, '.grabby');
  const preOpContractId = fileName.replace('.fc.md', '');
  savePreOpSummary(preOpGrabbyDir, preOpContractId, preOpSummary);
  ...
  recordRoutingOutcome({
    substep: persona.substep || 'scope_boundary',
    agentKey: persona.agentKey,
    outcome: 'success',
    source: persona.source || 'state',
  }, { cwd });
...
module.exports.buildPreOpSummary = buildPreOpSummary;
module.exports.savePreOpSummary = savePreOpSummary;
