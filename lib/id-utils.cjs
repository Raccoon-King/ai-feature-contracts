...
const WORK_ITEM_ID_RE = /([A-Za-z][A-Za-z0-9]+)-(\d+)/;
...
function extractContractId(content, fallbackFilename = '') {
  ...
  const fromContent = parseWorkItemId(idLineMatch ? idLineMatch[1] : '');
  if (fromContent) return fromContent;

  const fileBase = stripContractSuffix(path.basename(fallbackFilename || ''));
  const fromFilename = parseWorkItemId(fileBase);
...
function ensureIdMatchesFilename(contractPath) {
  ...
  const fileBase = stripContractSuffix(fileName);
  const filenameId = parseWorkItemId(fileBase);
