...
function normalizeRulesetRef(ruleset) {
  if (!ruleset || typeof ruleset !== 'object') return '';

  if (typeof ruleset.ref === 'string' && ruleset.ref.length > 0) {
    return ruleset.ref;
  }
...
function validateLock(lock) {
  ...
  lock.active.forEach((ruleset, index) => {
    const rulesetRef = normalizeRulesetRef(ruleset);

    if (!rulesetRef || typeof ruleset.category !== 'string') {
    ...
    if (!ruleset.version || typeof ruleset.version !== 'string') {
      throw new Error(`Active ruleset "${rulesetRef}" missing required field: version`);
    }

    if (!ruleset.hash || typeof ruleset.hash !== 'string') {
      throw new Error(`Active ruleset "${rulesetRef}" missing required field: hash`);
    }

    if (!ruleset.fetchedAt || typeof ruleset.fetchedAt !== 'string') {
      throw new Error(`Active ruleset "${rulesetRef}" missing required field: fetchedAt`);
    }

    // Validate hash format (sha256:...)
    if (!ruleset.hash.startsWith('sha256:')) {
      throw new Error(`Active ruleset "${rulesetRef}": hash must start with "sha256:"`);
    }
  });
...
function updateActiveRuleset(lock, ruleset) {
  ...
  if (!rulesetRef || !ruleset.version || !ruleset.hash) {
    throw new Error('Ruleset must have category, version, and hash');
  }

  const updated = { ...lock };
  const index = updated.active.findIndex((r) => normalizeRulesetRef(r) === rulesetRef);
  const [, derivedName = ''] = rulesetRef.split('/');

  const rulesetEntry = {
    category: rulesetRef,
    name: ruleset.name || derivedName,
...
function removeActiveRuleset(lock, category) {
  const updated = { ...lock };
  updated.active = updated.active.filter((r) => normalizeRulesetRef(r) !== category);
  return updated;
}
...
function findActiveRuleset(lock, category) {
  return lock.active.find((r) => normalizeRulesetRef(r) === category) || null;
}
