...
- If something is missing, state it and propose the smallest safe remediation.

Objective:
Complete bootstrap setup using ${setupContractTarget} and leave the repository ready for normal contract workflows.

Required flow:
1. Read ${setupContractTarget} and summarize pending items.
2. Run governance lifecycle for setup:
   - grabby validate SETUP-BASELINE.fc.md
   - grabby plan SETUP-BASELINE.fc.md
   - grabby approve SETUP-BASELINE.fc.md
3. Implement any required setup artifact updates in-repo (without broad refactors).
4. Produce completion notes that map directly to "Done When" checkboxes.
5. Mark setup baseline complete when criteria are satisfied:
...
`;
  ...
  function featureClose(id, options = {}) {
    if (isExternalLlmOnlyMode()) {
      blockStoryCompletionCommand('feature close');
      return;
    }
  ...
  function featureGc(action = 'list', id = null, options = {}) {
    ...
    if (isExternalLlmOnlyMode() && normalizedAction === 'archive') {
      blockStoryCompletionCommand('feature gc archive');
      return;
    }
