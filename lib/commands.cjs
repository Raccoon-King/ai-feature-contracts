...
const { generateBaselineContracts } = require('./assessment.cjs');
const {
  getSessionPath,
  buildSessionSummary,
...
`;
    ...
    if (isBrownfieldRepo) {
      ...
      logger.log('  2. Start bounded work with grabby ticket "<request>" or grabby task "<request>".');
      logger.log('  3. Validate and plan before editing brownfield code.');
    } else {
