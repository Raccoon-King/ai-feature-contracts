...
const { parseDuration } = require('../rulesets/sync.cjs');

function loadRuntimeConfig(cwd) {
  const config = require('../config.cjs');
  const runtimeConfig = config.loadConfig(cwd) || config.defaultConfig();
  const rulesetsConfig = config.loadRulesetsConfig(cwd, runtimeConfig);
  if (rulesetsConfig) {
    runtimeConfig.rulesets = rulesetsConfig;
  }
...
function createHealthRouter(context) {
  ...
  router.get('/config', (req, res) => {
    const appConfig = loadRuntimeConfig(context.cwd);
  ...
  router.put('/config', (req, res, next) => {
    try {
      ...
      const config = require('../config.cjs');
      const currentConfig = config.loadConfig(context.cwd) || config.defaultConfig();

      // Merge updates (shallow merge for safety)
      const newConfig = { ...currentConfig, ...updates };

      // Save config
      config.saveConfig(newConfig, context.cwd);
...
function checkRulesets(cwd) {
  try {
    const syncLock = require('../rulesets/common/sync-lock.cjs');
    const appConfig = loadRuntimeConfig(cwd);
    const lock = syncLock.readLock(appConfig.rulesets?.lockPath, cwd);
    ...
    try {
      maxAgeMs = parseDuration(appConfig.rulesets?.sync?.interval || '24h');
    } catch (_) {
      maxAgeMs = 24 * 60 * 60 * 1000;
    }
    const isStale = syncLock.isLockStale(lock, maxAgeMs);
