...
const { parseManifestFile, getAllRulesets, findRuleset, resolvePreset, listCategories, hasCategory } = require('./common/manifest-parser.cjs');
const { readLock, writeLock, updateActiveRuleset, removeActiveRuleset, findActiveRuleset, getLockAge, initLock, hashContent } = require('./common/sync-lock.cjs');
const { syncWithCentral, detectDrift, isGitAvailable } = require('./sync.cjs');
const { loadConfig, loadRulesetsConfig, saveRulesetsConfig } = require('../config.cjs');
...
function getRuntimeRulesetsConfig(cwd = process.cwd()) {
...
async function syncCommand(options = {}, cwd = process.cwd()) {
  const config = getRuntimeRulesetsConfig(cwd);

  if (!config || !config.source || !config.source.repo) {
    console.error(colors.red('❌ No central repository configured'));
    console.error('Add rulesets.source.repo to grabby.config.json or rulesets.config.json');
    return 1;
  }
  ...
  try {
    ...
    const lockPath = config.lockPath || '.grabby/rulesets/sync.lock.yaml';
    let lock = readLock(lockPath, cwd);

    if (!lock) {
      lock = initLock(config.source.repo, config.source.branch || 'main', lockPath, cwd);
    }

    // Update lock with sync info
    lock.lastSync = new Date().toISOString();
    lock.source = syncResult.source;
    lock.active = buildActiveLockEntries(syncResult.manifest, syncResult.manifestPath, config.active || []);
...
async function listCommand(options = {}, cwd = process.cwd()) {
  ...
  if (!config || !config.source || !config.source.repo) {
    ...
    console.error('Configure rulesets.source.repo in grabby.config.json or rulesets.config.json');
    return 1;
  }

  // Load manifest from cache
  const cacheDir = config.cacheDir || '.grabby/rulesets/cache';
  ...
  const lock = readLock(config.lockPath, cwd);
...
async function addCommand(rulesetRef, options = {}, cwd = process.cwd()) {
  ...
  if (config.active.includes(rulesetRef)) {
    console.log(colors.yellow(`⚠️  ${rulesetRef} is already active`));
    return 0;
  }

  // Add to active list
  config.active.push(rulesetRef);

  // Write updated config
  saveRulesetsConfig(config, cwd);
...
async function removeCommand(rulesetRef, options = {}, cwd = process.cwd()) {
  ...
  if (!config || !config.active || config.active.length === 0) {
    console.log(colors.yellow('⚠️  No active rulesets configured'));
    return 0;
  }

  const index = config.active.indexOf(rulesetRef);
  ...
  if (lock) {
    ...
    writeLock(updated, config.lockPath, cwd);
  }
...
async function statusCommand(options = {}, cwd = process.cwd()) {
  ...
  if (!config || !config.source || !config.source.repo) {
    ...
    console.log('Configure rulesets.source.repo in grabby.config.json or rulesets.config.json');
    return 1;
  }
...
async function presetCommand(presetName, options = {}, cwd = process.cwd()) {
  ...
  let addedCount = 0;
  preset.includes.forEach(ref => {
    if (!config.active.includes(ref)) {
      config.active.push(ref);
      addedCount++;
    }
  });
