...
function resolveContractPath(contractsDir, name) {
  ...
  for (const file of files) {
    ...
    if (content.includes(`**ID:** ${name}`) || content.toLowerCase().includes(`# fc: ${String(name).toLowerCase()}`)) return path.join(contractsDir, file);
  }
  return null;
}
...
function parseContract(content, fileName) {
  ...
  const id = content.match(/\*\*ID:\*\*\s*(FC-\d+)/)?.[1] || fileName.replace('.fc.md', '');
