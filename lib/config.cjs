...
function getRulesetsConfigPath(cwd = process.cwd()) {
  return path.join(cwd, 'rulesets.config.json');
}

function defaultRulesetsConfig() {
  return {
    source: {
      repo: '',
      branch: 'main',
    ...
    authoring: {
      ...
      guidanceSources: ['AGENTS.md', 'docs', '.cursor/rules', '.clinerules', '.continue/rules', 'README.md']
    },
    metadata: {
      enabled: true,
...
function defaultConfig() {
  return {
    $schema: getMetadataValue('docs.schemas.config') || 'https://grabby.dev/schemas/config.json',
    version: '1.0',
    jira: {
      enabled: false,
      host: '',
    ...
    rulesets: defaultRulesetsConfig(),
    dbGovernance: {
    ...
    gitGovernance: {
      ...
      allowDirectDefaultBranchCommits: false,
...
function normalizeRulesetsConfig(config) {
  if (!config || typeof config !== 'object') {
    return config;
  }

  const normalized = JSON.parse(JSON.stringify(deepMerge(defaultRulesetsConfig(), config)));

  if (typeof normalized.cacheDir === 'string') {
    normalized.cacheDir = normalizePortablePath(normalized.cacheDir);
  }
  if (typeof normalized.lockPath === 'string') {
    normalized.lockPath = normalizePortablePath(normalized.lockPath);
  }
  if (normalized.authoring?.protectedPath) {
    normalized.authoring.protectedPath = normalizePortablePath(normalized.authoring.protectedPath);
  }
  if (Array.isArray(normalized.local)) {
    normalized.local = normalizePathArray(normalized.local);
  }

  return normalized;
}

function deepMerge(defaults, userConfig) {
  const result = { ...defaults };
  for (const key of Object.keys(userConfig)) {
    if (userConfig[key] !== null && typeof userConfig[key] === 'object' && !Array.isArray(userConfig[key])) {
      result[key] = deepMerge(defaults[key] || {}, userConfig[key]);
    } else {
...
function normalizeConfig(config) {
  ...
  const normalized = JSON.parse(JSON.stringify(merged));
  ...
  if (normalized.dashboard && normalized.dashboard.port !== undefined) {
    const parsedPort = Number(normalized.dashboard.port);
    if (Number.isFinite(parsedPort)) {
      normalized.dashboard.port = Math.trunc(parsedPort);
    }
  }
  ...
  if (normalized.rulesets && typeof normalized.rulesets === 'object') {
    normalized.rulesets = normalizeRulesetsConfig(normalized.rulesets);
  }
...
function initConfig(cwd = process.cwd(), { force = false } = {}) {
  ...
  // Check if parent directory exists (handles cleanup race conditions in tests)
  if (!fs.existsSync(cwd)) return { created: false, file, skipped: true };
...
function loadConfig(cwd = process.cwd()) {
  const file = getConfigPath(cwd);
  // Check if parent directory exists first (handles cleanup race conditions)
  if (!fs.existsSync(cwd)) return normalizeConfig(defaultConfig());
  if (!fs.existsSync(file)) return null;
  return normalizeConfig(JSON.parse(fs.readFileSync(file, 'utf8')));
}

function loadRulesetsConfig(cwd = process.cwd(), baseConfig = null) {
  if (!fs.existsSync(cwd)) {
    return null;
  }

  let embedded = null;
  if (baseConfig && typeof baseConfig === 'object' && Object.prototype.hasOwnProperty.call(baseConfig, 'rulesets')) {
    embedded = baseConfig.rulesets;
  } else {
    const file = getConfigPath(cwd);
    if (fs.existsSync(file)) {
      const rawConfig = JSON.parse(fs.readFileSync(file, 'utf8'));
      if (rawConfig && typeof rawConfig === 'object' && Object.prototype.hasOwnProperty.call(rawConfig, 'rulesets')) {
        embedded = rawConfig.rulesets;
      }
    }
  }

  const sidecarPath = getRulesetsConfigPath(cwd);
  let sidecar = null;
  ...
  return normalizeRulesetsConfig(deepMerge(embedded || {}, sidecar || {}));
}
...
function saveRulesetsConfig(rulesetsConfig, cwd = process.cwd(), options = {}) {
  const normalized = normalizeRulesetsConfig(rulesetsConfig || {});
  const sidecarPath = getRulesetsConfigPath(cwd);
  const preferSeparate = options.preferSeparate === true || fs.existsSync(sidecarPath);

  if (preferSeparate) {
    fs.writeFileSync(sidecarPath, JSON.stringify(normalized, null, 2) + '\n');
    return sidecarPath;
  }

  const config = loadConfig(cwd) || defaultConfig();
  config.rulesets = normalized;
...
function validateConfig(config) {
  ...
  if (config.dashboard) {
    if ('port' in config.dashboard) {
      const port = config.dashboard.port;
      if (!Number.isInteger(port) || port < 1 || port > 65535) {
        errors.push('dashboard.port must be an integer between 1 and 65535');
      }
    }
  }
  ...
  if (config.rulesets?.authoring) {
    if ('enabled' in config.rulesets.authoring && typeof config.rulesets.authoring.enabled !== 'boolean') {
      errors.push('rulesets.authoring.enabled must be true or false');
    }
    if ('protectedPath' in config.rulesets.authoring && typeof config.rulesets.authoring.protectedPath !== 'string') {
      errors.push('rulesets.authoring.protectedPath must be a string');
    }
    if ('guidanceSources' in config.rulesets.authoring && !Array.isArray(config.rulesets.authoring.guidanceSources)) {
      errors.push('rulesets.authoring.guidanceSources must be an array');
    }
  }
  if (config.rulesets?.metadata) {
    if ('enabled' in config.rulesets.metadata && typeof config.rulesets.metadata.enabled !== 'boolean') {
      errors.push('rulesets.metadata.enabled must be true or false');
    }
    if ('location' in config.rulesets.metadata) {
      const allowed = ['frontmatter', 'sidecar'];
      if (!allowed.includes(String(config.rulesets.metadata.location))) {
        errors.push(`rulesets.metadata.location must be one of: ${allowed.join(', ')}`);
      }
    }
  }
  if (config.gitGovernance) {
    ...
    ['allowDirectDefaultBranchCommits', 'allowForcePush', 'allowRebaseAfterReviewOpen', 'requirePreflightBeforeExecute'].forEach((key) => {
...
module.exports = {
  getConfigPath,
  getRulesetsConfigPath,
  defaultConfig,
  defaultRulesetsConfig,
  initConfig,
  loadConfig,
  loadRulesetsConfig,
  saveConfig,
