...
rulesets:
  version: ""
  syncedAt: 2026-03-21T20:41:46.807Z
  snapshot: []
  driftChecks:
    - timestamp: 2026-03-21T20:41:46.808Z
      command: validate
      ...
      changes:
        ...
        - ruleset: tooling/ci-cd
          ...
          to: 1.0.0
          breaking: false
    - timestamp: 2026-03-21T20:42:10.530Z
      command: validate
      status: drift_detected
      action: warn
      changes:
        ...
        - ruleset: tooling/ci-cd
          ...
          to: 1.0.0
          breaking: false
    - timestamp: 2026-03-21T20:42:47.322Z
      command: validate
      status: drift_detected
      action: warn
      changes:
        ...
        - ruleset: tooling/ci-cd
          ...
          to: 1.0.0
          breaking: false
    - timestamp: 2026-03-21T20:43:02.376Z
      command: validate
      status: drift_detected
      action: warn
      changes:
        ...
        - ruleset: tooling/ci-cd
          ...
          to: 1.0.0
          breaking: false
    - timestamp: 2026-03-21T20:43:11.443Z
      command: plan
      status: drift_detected
      action: warn
      changes:
        ...
        - ruleset: tooling/ci-cd
          ...
          to: 1.0.0
          breaking: false
    - timestamp: 2026-03-21T20:44:33.765Z
      command: validate
      status: drift_detected
      action: warn
      changes:
        ...
        - ruleset: tooling/ci-cd
          ...
          to: 1.0.0
          breaking: false
    - timestamp: 2026-03-21T20:44:33.828Z
      command: plan
      status: drift_detected
      action: warn
      changes:
...
- Remove the orphaned calculator unit test that references a non-existent module
- Fix the website-sync workflow so it copies the docs that actually exist in this repository
...
## Directories
**Allowed:** `.github/`, `lib/api-routes/`, `tests/`, `contracts/`
**Restricted:** `node_modules/`, `.git/`, `dist/`, `coverage/`

## Files
| Action | Path | Reason |
|--------|------|--------|
| modify | `contracts/GRAB-CI-MERGE-001.fc.md` | Keep the governing contract aligned with the actual merge-fix scope |
...
| modify | `tests/api/rules.test.js` | Make rules API coverage deterministic and isolated from shared repo state |
| delete | `tests/unit/calculator.test.js` | Remove an orphaned test that references a missing calculator module |
| modify | `package.json` | Bundle all runtime dependencies for offline installation so packaging checks pass in CI |
...
- [ ] CI-facing tests remain deterministic across local and hosted environments
- [ ] Changes remain limited to the current merge blockers
- [ ] Rules API route logic matches the cache and manifest conventions already used by the rules CLI
- [ ] Offline packaging metadata reflects the actual runtime dependency set

## Done When
- [ ] Hosted CI no longer depends on a sub-100ms health check response
- [ ] The orphaned calculator unit test is removed from the suite
- [ ] Website sync copies docs from paths that exist in this repository
- [ ] `/v1/config` no longer returns 500 when repo config is missing during tests
...
## Testing
- Unit: `tests/api/health.test.js`
- Unit: `tests/api/rules.test.js`
- Unit: `tests/packaging.test.js`
