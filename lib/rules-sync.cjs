...
function cloneRepo(repoUrl, targetDir, options = {}) {
  ...
  const depth = options.shallow ? '--depth 1' : '';

  const command = `git clone ${depth} --branch ${branch} ${repoUrl} ${targetDir}`;
  return execGit(command, { timeout: options.timeout || 60000, silent: true });
}
...
function detectDrift(lock, manifest) {
  ...
  lock.active.forEach((activeRuleset) => {
    const [categoryName, rulesetName] = activeRuleset.category.split('/');
    const remote = remoteRulesets.find((r) => r.ref === activeRuleset.category);
  ...
  // Check for new rulesets (informational only)
  remoteRulesets.forEach((remote) => {
    const active = lock.active.find((a) => a.category === remote.ref);
