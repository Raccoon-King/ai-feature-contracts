const { parseWorkItemId, normalizeWorkItemId } = require('./id-utils.cjs');
...
function getMissingTicketFields(ticket) {
  const requireTicketId = Boolean(ticket?.requireTicketId);
  const normalized = {
  ...
  if (requireTicketId && !normalizeWhitespace(ticket?.ticketId)) {
    missing.unshift('ticketId');
  }
...
function mergeTicketInput(baseTicket, overrides = {}) {
  const rawTicketId = normalizeWhitespace(overrides.ticketId || baseTicket.ticketId || '');
  const normalizedTicketId = normalizeWorkItemId(rawTicketId);
  return {
    ticketId: normalizedTicketId || (rawTicketId ? rawTicketId.toUpperCase() : null),
    ...
    requireTicketId: overrides.requireTicketId !== undefined ? Boolean(overrides.requireTicketId) : Boolean(baseTicket.requireTicketId),
  };
}
...
function getWizardQuestions(ticket, options = {}) {
  ...
  const merged = mergeTicketInput(inferDraftTicket(rawRequest), {
    ...ticket,
    requireTicketId,
  });
  const questions = [];

  if (requireTicketId && !merged.ticketId) {
    questions.push({
      field: 'ticketId',
      prompt: 'Ticket ID (KEY-123)',
      defaultValue: '',
    });
  }
...
function applyWizardAnswer(ticket, field, value) {
  if (field === 'ticketId') {
    return mergeTicketInput(ticket, { ticketId: value });
  }
  if (field === 'dod') {
...
function validateTicket(ticket, options = {}) {
  const requireTicketId = options.requireTicketId !== undefined
    ? Boolean(options.requireTicketId)
    : Boolean(ticket?.requireTicketId);
  const effectiveTicket = mergeTicketInput({}, { ...ticket, requireTicketId });
  const missingFields = getMissingTicketFields(effectiveTicket);
  const errors = [];
  ...
  if (requireTicketId) {
    if (!effectiveTicket.ticketId) {
      errors.push('Ticket ID is required and must match KEY-123.');
    } else if (!parseWorkItemId(effectiveTicket.ticketId)) {
      errors.push('Ticket ID must match [A-Z][A-Z0-9]+-\\d+ and will be normalized to uppercase.');
    }
  }

  const dod = normalizeDodList(effectiveTicket?.dod);
  if (dod.length === 0) {
