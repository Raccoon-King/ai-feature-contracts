...
const { executeGraphQL } = require('./graphql.cjs');
...
function createAPIServer(context, options = {}) {
  ...
  const routes = {
    ...
    // GraphQL endpoint
    'POST /api/graphql': async (req, res) => {
      const body = await parseBody(req);
      const result = executeGraphQL(body.query, { contractsDir, grabbyDir });
      sendJSON(res, result, result.errors ? 400 : 200);
    },
    ...
    start: (port = 3456) => {
      return new Promise((resolve) => {
        server.listen(port, () => {
          ...
          console.log('  POST   /api/graphql             Run GraphQL query');
