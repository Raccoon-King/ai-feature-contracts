...
function detectProjectType(cwd) {
  ...
  if (
    hasPackageDependency(pkg, ['react', 'react-dom'])
    || ['src/App.jsx', 'src/App.tsx', 'src/index.jsx', 'src/index.tsx'].some((file) => fs.existsSync(path.join(cwd, file)))
  ) {
    detected.push('react');
  }

  if (
    hasPackageDependency(pkg, ['next'])
    || ['next.config.js', 'next.config.mjs', 'pages', 'app'].some((file) => fs.existsSync(path.join(cwd, file)))
  ) {
  ...
  if (
    ...
    || ['server.js', 'index.js', 'src/index.js'].some((file) => fs.existsSync(path.join(cwd, file)))
  ) {
    detected.push('node');
  }

  if (fs.existsSync(path.join(cwd, 'tsconfig.json'))) detected.push('typescript');
  if (['requirements.txt', 'setup.py', 'pyproject.toml', 'main.py'].some((file) => fs.existsSync(path.join(cwd, file)))) detected.push('python');
  if (['go.mod', 'main.go'].some((file) => fs.existsSync(path.join(cwd, file)))) detected.push('go');
  if (['Cargo.toml', 'src/main.rs'].some((file) => fs.existsSync(path.join(cwd, file)))) detected.push('rust');
