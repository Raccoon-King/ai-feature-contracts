...
function ensureGovernanceLock(context) {
  ...
  if (hadLock) {
    try {
      rawLock = fs.readFileSync(lockPath, 'utf8');
      existingLock = yaml.parse(rawLock) || {};
    } catch {
      existingLock = {};
    }
  }
  ...
  const lock = {
    ...existingLock,
    governance: {
      ...(existingLock.governance || {}),
      version: pkg.version || '0.0.0',
      profile: existingLock?.governance?.profile || 'default',
      rules_version: existingLock?.governance?.rules_version || 'v1',
    },
  };
  if (!fs.existsSync(context.grabbyDir)) fs.mkdirSync(context.grabbyDir, { recursive: true });
  const serializedLock = yaml.stringify(lock);
  const updated = !hadLock || rawLock !== serializedLock;
  if (updated) {
    fs.writeFileSync(lockPath, serializedLock);
  }
  return {
    path: lockPath,
    created: !hadLock,
    updated: hadLock && updated,
  };
}
...
function isPlaceholderScript(scriptName, script) {
  if (!script || typeof script !== 'string') return false;
  const normalized = script.toLowerCase();
  // Check for echo-based no-op scripts
  if (/^\s*echo\b/i.test(script) && /\bno\s+(lint|build|test)\s+configured\b/i.test(script)) {
    return true;
  }
  // Check for explicit TODO/placeholder markers
  if (/\b(todo|placeholder|not\s+implemented)\b/i.test(normalized)) {
    return true;
  }
  // Check for exit 0 only scripts
  if (/^\s*exit\s+0\s*$/.test(script.trim())) {
    return true;
  }
  return false;
}
...
function createCommandHandlers({
  ...
  loadRulesetIntegration = () => require('./rulesets/contract-integration.cjs'),
  createPluginManagerImpl = () => {
    const { createPluginManager } = require('./plugins/index.cjs');
    return createPluginManager({
      cwd: context.pkgRoot,
      pluginsDir: path.join(context.pkgRoot, 'lib', 'plugins'),
    });
  },
}) {
  const c = {
  ...
  function getPluginManager() {
    ...
    try {
      pluginManager = createPluginManagerImpl();
    } catch (error) {
      logger.log(c.warn(`Warning: Plugin system unavailable: ${error.message}`));
      pluginManager = null;
    }
  ...
  function hasRegisteredPlugin(pluginId) {
    const manager = getPluginManager();
    if (!manager || typeof manager.getPlugin !== 'function') {
      return false;
    }
    return Boolean(manager.getPlugin(pluginId));
  }

  async function dispatchPluginHooks(commandName, payload = {}) {
    const manager = getPluginManager();
    if (!manager || typeof manager.dispatchHook !== 'function') {
    ...
    const commandResult = await manager.dispatchHook('onCommand', hookContext);
    aggregate.push(...(commandResult.results || []));
    if (commandResult.cancelled) {
      return { ...commandResult, results: aggregate };
    }

    if (!specificHook) {
      return { cancelled: false, results: aggregate };
    }

    const specificResult = await manager.dispatchHook(specificHook, hookContext);
    aggregate.push(...(specificResult.results || []));
    return {
      cancelled: specificResult.cancelled === true,
      reason: specificResult.reason || null,
      results: aggregate,
    };
  }
  ...
  async function performRulesetSyncCheck(commandName, contractPath = null) {
    try {
      let integration;
      try {
        integration = loadRulesetIntegration();
      } catch (error) {
        logger.log(c.warn(`Warning: Ruleset integration unavailable: ${error.message}`));
        return true;
      }

      const { performSyncCheck, updateContractDriftCheck } = integration || {};
      if (typeof performSyncCheck !== 'function') {
        return true;
      }

      const checkResult = await performSyncCheck(commandName, { logger }, context.cwd);
      const rulesetsConfig = loadRulesetsConfig(context.cwd, loadRepoConfig(context.cwd) || {}) || {};
      const blockingEnabled = rulesetsConfig.sync?.blocking === true;

      // Update contract metadata if provided
      if (contractPath && checkResult && !checkResult.skipped && typeof updateContractDriftCheck === 'function') {
        try {
          await updateContractDriftCheck(contractPath, checkResult, commandName, context.cwd);
        } catch (error) {
          // Non-fatal - just log warning
          logger.log(c.warn(`Warning: Failed to update contract metadata: ${error.message}`));
        }
      }

      // Core commands warn by default; explicit blocking is opt-in.
      if (checkResult.blocked) {
        if (blockingEnabled) {
          logger.log(c.error('Execution blocked by ruleset sync check'));
          exit(1);
          return false;
        }
        logger.log(c.warn('Warning: Ruleset drift detected, but blocking is disabled. Continuing.'));
      }

      return true;
    } catch (error) {
      // Non-fatal error - log warning and continue
      logger.log(c.warn(`Warning: Ruleset sync check failed: ${error.message}`));
      return true;
    }
  }
  ...
  function formatSemverVersion(version) {
    ...
    const prerelease = Array.isArray(parsed.prerelease) && parsed.prerelease.length > 0
      ? `-${parsed.prerelease.join('.')}`
      : '';
    return `${parsed.major}.${parsed.minor}.${parsed.patch}${prerelease}`;
  }

  function bumpSemverVersion(version, bumpType = 'patch') {
    ...
    return formatSemverVersion(next);
  }

  function inferContractVersionBump(content = '') {
    if (/\*\*Breaking API Change Approved:\*\*\s*(yes|true)/i.test(content)) {
      return 'major';
    }

    if (
      /\*\*Data Change:\*\*\s*(yes|true)/i.test(content)
      || /\*\*API Change:\*\*\s*(yes|true)/i.test(content)
      || /\*\*Dependency Change:\*\*\s*(yes|true)/i.test(content)
    ) {
      return 'minor';
    }
  ...
  function readRepoVersion() {
    ...
    try {
      ...
      return formatSemverVersion(pkg?.version);
    } catch {
      return null;
    }
  }

  function buildPlanVersionTransition(content = '') {
    const currentVersion = readRepoVersion();
    if (!currentVersion) {
      return null;
    }

    const nextVersion = bumpSemverVersion(currentVersion, inferContractVersionBump(content));
    if (!nextVersion) {
      return null;
    }
  ...
  async function init(options = {}) {
    ...
    const lockResult = ensureGovernanceLock(context);
    const lockPath = lockResult.path;
    ...
    (lockResult.created ? noteCreated : (lockResult.updated ? noteUpdated : notePreserved))(path.relative(context.cwd, lockPath).replace(/\\/g, '/'));
...
`;
  ...
  async function create(name) {
    ...
    try {
      ...
      const hookResult = await dispatchPluginHooks('create', {
        contractPath: result.filePath,
        file: result.fileName,
      });
      if (hookResult.cancelled) {
        logger.log(c.warn(`Contract post-create processing cancelled by plugin${hookResult.reason ? `: ${hookResult.reason}` : '.'}`));
      }
  ...
  async function validate(file) {
    ...
    const hookResult = await dispatchPluginHooks('validate', { contractPath: filePath, file });
    if (hookResult.cancelled) {
      logger.log(c.warn(`Validation cancelled by plugin${hookResult.reason ? `: ${hookResult.reason}` : '.'}`));
      return;
    }

    if (!hasRegisteredPlugin('rulesets')) {
      const syncCheckPassed = await performRulesetSyncCheck('validate', filePath);
      if (!syncCheckPassed) {
        return;
      }
    }
  ...
  async function plan(file) {
    ...
    const hookResult = await dispatchPluginHooks('plan', { contractPath: filePath, file });
    if (hookResult.cancelled) {
      logger.log(c.warn(`Plan generation cancelled by plugin${hookResult.reason ? `: ${hookResult.reason}` : '.'}`));
      return;
    }

    if (!hasRegisteredPlugin('rulesets')) {
      const syncCheckPassed = await performRulesetSyncCheck('plan', filePath);
      if (!syncCheckPassed) {
        return;
      }
    }
    ...
    const planVersion = buildPlanVersionTransition(content);
    const planData = {
      contract: fileName,
      phase: 'plan',
    ...
    // Warn about protected path targets
    const protectedPathTargets = files
      .filter((entry) => {
        const targetPath = path.resolve(context.cwd, entry.path);
        return isProtectedRulesPath(targetPath, context.cwd);
      })
      .map((entry) => entry.path);

    if (protectedPathTargets.length > 0) {
      logger.log(c.warn('? Warning: Plan targets protected shared rules path:'));
      protectedPathTargets.forEach((p) => logger.log(c.warn(`  - ${p}`)));
      logger.log(c.warn('These files cannot be modified during normal execution.'));
      logger.log(c.warn('Use \'grabby rules generate\' or \'grabby rules update\' instead.'));
      logger.log('');
    }
  ...
  async function execute(file, options = {}) {
    ...
    const hookResult = await dispatchPluginHooks('execute', { contractPath: filePath, file, options });
    if (hookResult.cancelled) {
      logger.log(c.warn(`Execution cancelled by plugin${hookResult.reason ? `: ${hookResult.reason}` : '.'}`));
      return;
    }

    if (!hasRegisteredPlugin('rulesets')) {
      const syncCheckPassed = await performRulesetSyncCheck('execute', filePath);
      if (!syncCheckPassed) {
        return;
      }
    }
    ...
    const protectedPathViolations = planEntries
      .filter((entry) => {
        const targetPath = path.resolve(context.cwd, entry.path);
        return isProtectedRulesPath(targetPath, context.cwd);
      })
    ...
    if (protectedPathViolations.length > 0) {
      logger.log('? Protected path violation detected:');
      logger.log('The following files target the protected shared rules directory:');
      protectedPathViolations.forEach((p) => logger.log(`  - ${p}`));
      logger.log('');
      logger.log('Use \'grabby rules generate\' or \'grabby rules update\' to modify shared rules.');
      logger.log('Normal contract execution cannot write to this protected location.');
      exit(1);
      return;
    }
  ...
  function preflight(options = {}) {
    const { quick = false, all = false, verbose = false } = options;
    const errors = [];
    const warnings = [];
    const passed = [];
    const repoConfig = loadRepoConfig(context.cwd) || {};
    ...
    logger.log('');
    logger.log(c.heading('Local Preflight Checks'));
    logger.log('-'.repeat(50));

    // 1. Git state check (branch policy, clean workspace)
    try {
      const summary = summarizeGitStatus(context.cwd, execSyncImpl, repoConfig);
      const branchCheck = preflightGitState({
        cwd: context.cwd,
        execSyncImpl,
        config: repoConfig,
        enforceBranchPolicyOnly: true,
      ...
      if (branchCheck.valid) {
        passed.push(`Branch policy: ${summary.branch}`);
      } else {
        branchCheck.errors.forEach((err) => errors.push(`Branch: ${err}`));
      }

      if (summary.dirty) {
        warnings.push(`Working tree has uncommitted changes (${summary.stagedCount} staged, ${summary.unstagedCount} unstaged)`);
      } else {
        passed.push('Working tree: clean');
      }

      if (summary.behind > 0) {
        warnings.push(`Branch is ${summary.behind} commit(s) behind ${summary.upstream || summary.defaultBranch}`);
      }
    } catch (err) {
      if (!String(err.message || '').includes('Not a git repository')) {
        errors.push(`Git: ${err.message}`);
      } else {
        warnings.push('Not a git repository (git checks skipped)');
      }
    }

    // 2. Contract validation (all active contracts)
    const contracts = featuresLib.listContractFeatures(context.cwd);
    const activeContracts = contracts.filter((c) => ['draft', 'approved', 'executing'].includes(c.status));

    if (activeContracts.length > 0) {
      activeContracts.forEach((contract) => {
        try {
          // contract.contractPath is relative, so join with cwd
          const contractPath = contract.contractPath
            ? path.join(context.cwd, contract.contractPath)
            : path.join(context.contractsDir, `${contract.id}.fc.md`);
          if (!fs.existsSync(contractPath)) {
            errors.push(`Contract missing: ${contract.id}`);
            return;
          }
          const content = fs.readFileSync(contractPath, 'utf8');
          const validation = validateContract(content);
          ...
          if (!validation.valid) {
            errors.push(`Contract ${contract.id}: ${validation.errors.slice(0, 2).join('; ')}`);
          } else {
            passed.push(`Contract ${contract.id}: valid`);
          }
          ...
          if (policyViolations.length > 0) {
            policyViolations.forEach((v) => warnings.push(`${contract.id}: ${v}`));
          }
        } catch (err) {
      ...
      passed.push('Contracts: none active (ok)');
    }

    // 3. Dependency checks (bundleDependencies, lockfile)
    const pkgPath = path.join(context.cwd, 'package.json');
    const lockPath = path.join(context.cwd, 'package-lock.json');
    if (fs.existsSync(pkgPath)) {
      try {
        const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
        ...
        // Check bundleDependencies exist in dependencies
        const missingBundled = bundled.filter((name) => !deps[name]);
        if (missingBundled.length > 0) {
          errors.push(`bundleDependencies not in dependencies: ${missingBundled.join(', ')}`);
        } else if (bundled.length > 0) {
          passed.push(`bundleDependencies: ${bundled.length} declared`);
        }

        // Check lockfile exists and is in sync (simple check)
        if (!fs.existsSync(lockPath)) {
          warnings.push('package-lock.json missing (run npm install)');
        } else if (!quick) {
          const lockStat = fs.statSync(lockPath);
          const pkgStat = fs.statSync(pkgPath);
          if (pkgStat.mtimeMs > lockStat.mtimeMs) {
            warnings.push('package.json newer than package-lock.json (run npm install)');
          } else {
            passed.push('Lockfile: in sync');
          }
        }
      } catch (err) {
    ...
    // 4. npm audit (unless quick mode)
    if (!quick && fs.existsSync(pkgPath)) {
      try {
        const auditResult = execSyncImpl('npm audit --audit-level=high --json', {
          cwd: context.cwd,
          stdio: 'pipe',
          encoding: 'utf8',
        });
        const auditData = JSON.parse(auditResult);
        const vulnCount = auditData.metadata?.vulnerabilities?.high || 0;
        const critCount = auditData.metadata?.vulnerabilities?.critical || 0;
        if (vulnCount > 0 || critCount > 0) {
          errors.push(`npm audit: ${critCount} critical, ${vulnCount} high vulnerabilities`);
        } else {
          passed.push('npm audit: no high/critical vulnerabilities');
        }
      } catch (err) {
        ...
        if (err.stdout) {
          try {
            ...
            const critCount = auditData.metadata?.vulnerabilities?.critical || 0;
            if (vulnCount > 0 || critCount > 0) {
              warnings.push(`npm audit: ${critCount} critical, ${vulnCount} high vulnerabilities found`);
            }
          } catch {
            warnings.push('npm audit: check failed (run npm audit manually)');
          }
        } else {
          warnings.push('npm audit: unavailable');
        }
      }
    }

    // 5. Check for required scripts (lint, test)
    if (all && fs.existsSync(pkgPath)) {
      try {
        const pkg = JSON.parse(fs.readFileSync(pkgPath, 'utf8'));
        const scripts = pkg.scripts || {};
        const requiredChecks = getRequiredChecks(repoConfig);

        requiredChecks.forEach((check) => {
          if (check === 'guard') return; // guard is internal
          if (!scripts[check]) {
            warnings.push(`Missing npm script: ${check}`);
          } else if (isPlaceholderScript(check, scripts[check])) {
            warnings.push(`Placeholder npm script: ${check}`);
          } else {
            passed.push(`npm script: ${check} defined`);
          }
        });
      } catch {}
    }
    ...
    if (passed.length > 0 && verbose) {
      passed.forEach((msg) => logger.log(c.success(`? ${msg}`)));
    } else if (passed.length > 0) {
      logger.log(c.success(`? ${passed.length} checks passed`));
    }

    if (warnings.length > 0) {
      logger.log('');
      warnings.forEach((msg) => logger.log(c.warn(`? ${msg}`)));
    }

    if (errors.length > 0) {
      logger.log('');
      errors.forEach((msg) => logger.log(c.error(`? ${msg}`)));
      logger.log('');
      logger.log('Preflight failed. Fix issues above before pushing.');
      exit(1);
      return;
    }

    logger.log('');
    logger.log(c.success('? Preflight passed.'));
    if (!quick) {
      logger.log(c.dim('Run before push: npm run preflight'));
    }
    logger.log('');
  }
