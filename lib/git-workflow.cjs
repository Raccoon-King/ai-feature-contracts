...
function preflightGitState({ cwd, execSyncImpl, config = {}, contractId = null, expectedBranch = null, hasPlan = false, requiredChecksKnown = {}, policyTriggered = false }) {
  ensureGitRepository(execSyncImpl, cwd);
  const state = loadGitState(cwd, config);
  const summary = summarizeGitStatus(cwd, execSyncImpl, config);
  const errors = [];
