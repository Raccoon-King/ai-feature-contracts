...
async function runTaskBreakdownWorkflow(rl, seedRequest = '', options = {}) {
  ...
  const createRequest = inferCreateRequest(request);
  ...
  const objective = nonInteractive
    ...
    : await resolveInputValue(rl, input.objective, 'What outcome do you want?', request);
  ...
  const doneWhenInput = input.doneWhen
    ? input.doneWhen.join(', ')
    : nonInteractive
      ? 'Tests pass, lint passes, and the bounded outcome is complete'
      : await prompt(
        rl,
        'Definition of done',
        'Tests pass, lint passes, and the bounded outcome is complete'
  ...
  const testing = nonInteractive
    ...
    : await resolveInputValue(rl, input.testing, 'Testing approach', defaults.testing);
  ...
  const fileName = `${slug(taskName)}.fc.md`;
  const contractPath = path.join(CONTRACTS_DIR, fileName);
  const contractContent = buildTaskContract({
    taskName,
    id: genId(),
    objective,
    scopeItems,
  ...
  const briefPath = getTaskBriefPath(CONTRACTS_DIR, taskName);
  ...
  console.log(c.heading('-'.repeat(50)));
  console.log(`\nContract: ${c.info(`contracts/${fileName}`)}`);
  console.log(`Brief: ${c.info(`contracts/${path.basename(briefPath)}`)}`);
  console.log(`Template: ${c.dim(createRequest.templateName)}`);
  if (executionBriefPath) {
    console.log(`Execution: ${c.info(`contracts/${path.basename(executionBriefPath)}`)}`);
  }
  if (auditChecklistPath) {
    console.log(`Audit: ${c.info(`contracts/${path.basename(auditChecklistPath)}`)}`);
  }
