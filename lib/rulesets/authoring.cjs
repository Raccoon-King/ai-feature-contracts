...
const { discoverRulesetCandidates, resolveInputFiles, summarizeFiles } = require('./builder.cjs');
...
function getProtectedRulesPath(cwd = process.cwd()) {
  ...
  const rulesetsConfig = loadRulesetsConfig(cwd, repoConfig);
  const configuredPath = rulesetsConfig?.authoring?.protectedPath;
  if (configuredPath) {
...
function getGuidanceSources(cwd = process.cwd()) {
  const repoConfig = loadConfig(cwd);
  const rulesetsConfig = loadRulesetsConfig(cwd, repoConfig);
  const configuredSources = rulesetsConfig?.authoring?.guidanceSources;
...
function isAuthoringEnabled(cwd = process.cwd()) {
  const repoConfig = loadConfig(cwd);
  const rulesetsConfig = loadRulesetsConfig(cwd, repoConfig);
  return rulesetsConfig?.authoring?.enabled !== false;
}

/**
 * Generate a shared ruleset as a draft (without writing to disk)
 * @param {Object} options - Generation options
 * @param {string} options.cwd - Current working directory
 * @param {string} options.goal - Goal for the ruleset
 * @param {string} options.title - Title for the ruleset
 * @param {string[]} options.sources - Specific sources to use (optional)
 * @returns {Promise<{content: string, metadata: object, verification: object}>}
 */
async function generateSharedRulesDraft(options = {}) {
  const cwd = options.cwd || process.cwd();
  const title = options.title || 'Shared Project Rules';
  ...
  return {
    ...
    metadata: {
      ...
      sourcesUsed: guidanceFiles.length,
      sources: guidanceFiles.map(f => path.relative(cwd, f).replace(/\\/g, '/')),
      usedProvider: Boolean(provider),
    },
    verification: {
      valid: true,
      errors: [],
      warnings: guidanceFiles.length === 0 ? ['No guidance files found'] : [],
    },
  };
}

/**
 * Generate an update to an existing shared ruleset as a draft
 * @param {Object} options - Update options
 * @param {string} options.cwd - Current working directory
 * @param {string} options.rulesetPath - Path to the ruleset to update
 * @param {string} options.goal - What to update/change
 ...
 * @returns {Promise<{content: string, metadata: object, verification: object}>}
 */
async function generateUpdateDraft(options = {}) {
  const cwd = options.cwd || process.cwd();
  ...
  return {
    ...
    metadata: {
      originalPath: path.relative(cwd, absolutePath).replace(/\\/g, '/'),
      sourcesUsed: guidanceFiles.length,
      sources: guidanceFiles.map(f => path.relative(cwd, f).replace(/\\/g, '/')),
      usedProvider: Boolean(provider),
    },
