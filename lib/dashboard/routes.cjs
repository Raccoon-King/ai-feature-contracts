...
function buildHistoryItem(feature) {
  return {
    ...
    splitStory: feature.splitStory === true,
  };
}

function buildHistoryDetail(id, cwd) {
  const normalizedId = String(id || '').trim().toUpperCase();
  const feature = features.listArchivedFeatures(cwd)
    .find((entry) => String(entry.id || '').trim().toUpperCase() === normalizedId);

  if (!feature) {
    return null;
  }

  const historyPlanPath = path.join(cwd, '.grabby', 'history', `${normalizedId}.plan.yaml`);
  const relativePlanPath = fs.existsSync(historyPlanPath)
    ? path.relative(cwd, historyPlanPath).replace(/\\/g, '/')
    : null;
  const archiveText = feature.archivePath && String(feature.archivePath).endsWith('.bundle.md')
    ? readTextIfExists(path.join(cwd, feature.archivePath))
    : null;

  return {
    ...buildHistoryItem(feature),
    planPath: relativePlanPath,
    planText: readTextIfExists(historyPlanPath),
    archiveText,
  };
}
...
function createRouter(options = {}) {
  ...
  router.get('/history/:id', (req, res) => {
    ...
    const detail = buildHistoryDetail(id, cwd);
    if (!detail) {
      res.status(404).json({ ok: false, error: 'Archived contract not found' });
      return;
    }

    res.json({ ok: true, history: detail });
  });
