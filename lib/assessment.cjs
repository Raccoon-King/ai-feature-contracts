...
const { buildComponentIndex } = require('./component-indexer.cjs');
const { buildLanguageSetup } = require('./language-setup.cjs');
...
function deriveProjectContext(assessment, summary = '', options = {}) {
  ...
  // Build component index for React/Vue projects
  let componentPreferences = null;
  if (options.cwd) {
    try {
      const componentIndex = buildComponentIndex(options.cwd, { frontend: { frameworks: assessment.projectTypes, roots: assessment.projectDirs } });
      if (componentIndex.preferenceHint) {
        componentPreferences = {
          hint: componentIndex.preferenceHint,
          framework: componentIndex.framework,
          componentCount: componentIndex.components?.length || 0,
          designSystem: componentIndex.designSystem,
        };
      }
    } catch {
      // Component indexing optional
    }
  }

  // Build language setup for non-Node projects
  let languageSetup = null;
  if (options.cwd) {
    try {
      const setup = buildLanguageSetup(options.cwd);
      if (setup.languages.length > 0) {
        languageSetup = {
          primary: setup.primary,
          languages: setup.languages,
          tooling: setup.generalTooling,
        };
      }
    } catch {
  ...
  return {
    ...
    // New: component and language awareness
    componentPreferences,
    languageSetup,
  };
}
...
async function generateProjectContextArtifact({ cwd, outputPath = path.join(cwd, '.grabby', 'project-context.json') }) {
  ...
  const projectContext = deriveProjectContext(assessment, summary, { cwd });
