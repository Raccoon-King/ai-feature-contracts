...
const { ensureDir, readJsonSafe, writeJsonAtomic, readYamlSafe, writeYamlAtomic } = require('./fs-utils.cjs');
...
function initFeaturesFile(baseDir = process.cwd()) {
  ...
  if (!fs.existsSync(featuresPath)) {
    ...
    writeYamlAtomic(featuresPath, initial);
  }
...
function getFeatureArtifactPaths(id, baseDir = process.cwd()) {
  ...
  const discovered = listContractFeatures(baseDir).find((feature) => feature.id === normalizedId);
  const discoveredContractPath = discovered?.contractPath
    ? path.join(baseDir, normalizeRepoPath(discovered.contractPath))
    : null;
  const contractPath = discoveredContractPath && fs.existsSync(discoveredContractPath)
    ? discoveredContractPath
    : (getCandidateContractsDirs(baseDir).find((dir) => fs.existsSync(path.join(dir, `${normalizedId}.fc.md`)))
      ? path.join(getCandidateContractsDirs(baseDir).find((dir) => fs.existsSync(path.join(dir, `${normalizedId}.fc.md`))), `${normalizedId}.fc.md`)
      : path.join(getActiveContractsDir(baseDir), `${normalizedId}.fc.md`));
  const activeDir = path.dirname(contractPath);
  const contractStem = path.basename(contractPath).replace(/\.fc\.md$/i, '');
  const idPlanPath = path.join(activeDir, `${normalizedId}.plan.yaml`);
  const idAuditPath = path.join(activeDir, `${normalizedId}.audit.md`);
  const stemPlanPath = path.join(activeDir, `${contractStem}.plan.yaml`);
  const stemAuditPath = path.join(activeDir, `${contractStem}.audit.md`);
  return {
    activeDir,
    contractPath,
    planPath: fs.existsSync(idPlanPath) ? idPlanPath : stemPlanPath,
    auditPath: fs.existsSync(idAuditPath) ? idAuditPath : stemAuditPath,
  };
}
...
function parseTargetedRelease(content) {
  const value = extractMetadataLine(content, 'Targeted Release');
  if (!value) {
    return null;
  }
...
function parseGarbageCollect(content) {
  const value = extractMetadataLine(content, 'Garbage Collect');
  if (!value) {
    return false;
  }

  return /^(yes|true|1|on|enabled)$/i.test(value.trim());
}

function parseSplitStory(content) {
  const value = extractMetadataLine(content, 'Split Story');
  if (!value) {
    return false;
  }
...
function parseRunOrder(content) {
  // Match **Run Order:** N pattern
  const match = content.match(/\*\*Run Order:\*\*\s*(\d+)/i);
  if (match) {
    return parseInt(match[1], 10);
  }
  // Default to 0 (highest priority) for contracts without explicit run order
  return 0;
}
...
function parseFeatureContract(content, contractPath, baseDir = process.cwd()) {
  ...
  let id;
  try {
    id = extractContractId(content, fileName);
  } catch {
    id = fileName.replace(/\.fc\.md$/i, '').toUpperCase();
  }
  const title = (
  ...
  const runOrder = parseRunOrder(content);
  const targetedRelease = parseTargetedRelease(content);
  const garbageCollect = parseGarbageCollect(content);
  const splitStory = parseSplitStory(content);
  const type = (
...
function listContractFeatures(baseDir = process.cwd()) {
  ...
  // Sort by runOrder first (ascending), then by ID for stable ordering
  return features.sort((a, b) => {
    const orderDiff = (a.runOrder || 0) - (b.runOrder || 0);
      ...
      if (history?.entries) {
        for (const entry of history.entries) {
          results.push({
            ...
            type: entry.type,
            files: entry.files,
            targetedRelease: entry.targetedRelease || null,
            garbageCollect: entry.garbageCollect === true,
            splitStory: entry.splitStory === true,
          });
        }
      }
    }
...
function listGarbageCandidates(baseDir = process.cwd(), options = {}) {
  ...
  const existingIndex = loadFeatureIndex(baseDir);
  const existingById = new Map((existingIndex?.features || []).map((feature) => [feature.id, feature]));
  const baselineIds = new Set(['SYSTEM-BASELINE', 'PROJECT-BASELINE', 'SETUP-BASELINE']);

  return active
    .map((feature) => mergeFeatureMetadata(feature, existingById.get(feature.id)))
    .map((feature) => {
      if (baselineIds.has(String(feature.id || '').toUpperCase())) {
        return null;
      }
...
function recordGarbageDisposition(featureId, disposition, baseDir = process.cwd(), options = {}) {
  ...
  if (normalizedDisposition === 'archive') {
    return createArchiveBundle(normalizedId, baseDir, {
      ...options,
...
function addToHistory(entry, baseDir = process.cwd()) {
  ...
  if (fs.existsSync(historyFile)) {
    const parsed = yaml.parse(fs.readFileSync(historyFile, 'utf8'));
...
function createCompactHistoryEntry(parsed, contractContent, planData, closedAt) {
  ...
  return {
    ...
    targetedRelease: parsed.targetedRelease || undefined,
    garbageCollect: parsed.garbageCollect === true ? true : undefined,
    splitStory: parsed.splitStory === true ? true : undefined,
    closedAt,
  };
}
...
function createArchiveBundle(featureId, baseDir = process.cwd(), options = {}) {
  ...
  if (!allowIncomplete && !['complete', 'completed'].includes(parsed.status)) {
  ...
  // Archive the plan YAML to history directory (preserve execution context)
  const historyDir = path.join(baseDir, HISTORY_DIR);
  let archivedPlanPath = null;
  if (artifactPaths.planPath && fs.existsSync(artifactPaths.planPath)) {
    const planFileName = `${normalizedId}.plan.yaml`;
    archivedPlanPath = path.join(historyDir, planFileName);
    fs.copyFileSync(artifactPaths.planPath, archivedPlanPath);
  }

  // Remove all feature-scoped artifacts generated from the same feature ID.
  // Plan YAML is archived above, so it's safe to delete from active directory.
  const artifactStems = new Set([
    normalizedId,
    path.basename(artifactPaths.contractPath).replace(/\.fc\.md$/i, ''),
  ]);
  ...
  explicitArtifacts.forEach((artifactPath) => {
    if (artifactPath && fs.existsSync(artifactPath)) {
      fs.rmSync(artifactPath, { force: true });
    }
  });

  artifactStems.forEach((stem) => {
    ARCHIVABLE_ARTIFACT_SUFFIXES.forEach((suffix) => {
      const artifactPath = path.join(artifactPaths.activeDir, `${stem}${suffix}`);
  ...
  const refreshed = refreshFeatureIndex(baseDir);
  const activeDirEntries = fs.existsSync(artifactPaths.activeDir)
    ? fs.readdirSync(artifactPaths.activeDir)
    : [];
  if (activeDirEntries.length === 0) {
    fs.rmSync(artifactPaths.activeDir, { recursive: true, force: true });
  }

  return {
    id: normalizedId,
    historyFile: path.relative(baseDir, historyFile).replace(/\\/g, '/'),
    archivedPlanPath: archivedPlanPath ? path.relative(baseDir, archivedPlanPath).replace(/\\/g, '/') : null,
    closedAt,
    indexPath: path.relative(baseDir, refreshed.indexPath).replace(/\\/g, '/'),
  };
}

function isBaselineStoryId(id) {
  return ['SYSTEM-BASELINE', 'PROJECT-BASELINE', 'SETUP-BASELINE'].includes(String(id || '').trim().toUpperCase());
}

/**
 * Update the run order for a contract
 * @param {string} contractId - The contract ID
 * @param {number} newRunOrder - The new run order value
 * @param {string} baseDir - Base directory
 * @returns {object} Updated contract info
 */
function updateContractRunOrder(contractId, newRunOrder, baseDir = process.cwd()) {
  const normalizedId = String(contractId || '').trim().toUpperCase();
  const order = parseInt(newRunOrder, 10);
  if (!Number.isFinite(order) || order < 0) {
    throw new Error(`Invalid run order: ${newRunOrder}`);
  }

  const feature = listContractFeatures(baseDir).find((f) => f.id === normalizedId);
  if (!feature) {
    throw new Error(`Contract ${normalizedId} not found`);
  }

  const contractPath = path.join(baseDir, feature.contractPath);
  let content = fs.readFileSync(contractPath, 'utf8');

  // Update existing Run Order or add it after Status
  const runOrderPattern = /(\*\*Run Order:\*\*\s*)\d+/i;
  const statusPattern = /(\*\*Status:\*\*\s*[^\n\r|]+)/i;

  if (runOrderPattern.test(content)) {
    content = content.replace(runOrderPattern, `$1${order}`);
  } else if (statusPattern.test(content)) {
    // Add Run Order after Status on the same line
    content = content.replace(statusPattern, `$1 | **Run Order:** ${order}`);
  } else {
    throw new Error(`Unable to locate Status line in contract ${normalizedId}`);
  }
...
/**
 * Reorder multiple contracts at once
 * @param {Array<{id: string, runOrder: number}>} updates - Array of contract ID and new run order pairs
 * @param {string} baseDir - Base directory
 * @returns {object} Result with updated contracts
 */
function reorderContracts(updates, baseDir = process.cwd()) {
  ...
  for (const { id, runOrder } of updates) {
    try {
      const result = updateContractRunOrder(id, runOrder, baseDir);
      results.push(result);
    } catch (error) {
...
function garbageCollectCompletedStories(baseDir = process.cwd(), options = {}) {
  const completedStories = listContractFeatures(baseDir)
    .filter((feature) => ['complete', 'completed'].includes(normalizeContractStatus(feature.status)))
    .filter((feature) => !isBaselineStoryId(feature.id))
    .sort((a, b) => a.id.localeCompare(b.id));

  const archived = [];
  const failed = [];
  completedStories.forEach((feature) => {
    try {
      archived.push(createArchiveBundle(feature.id, baseDir, options));
    } catch (error) {
      failed.push({ id: feature.id, error: error.message });
    }
  });
...
module.exports = {
  ...
  garbageCollectCompletedStories,
  parseRunOrder,
  updateContractRunOrder,
  reorderContracts,
};
