...
const { parseRulesetWithFrontmatter, extractMetadata, DEFAULT_METADATA } = require('./frontmatter.cjs');
...
function parseRuleset(content, source = 'unknown') {
  ...
  const { metadata, content: markdownContent, validation } = parseRulesetWithFrontmatter(content);

  // Extract name from content (fallback to metadata.id)
  const nameMatch = markdownContent.match(/^#\s+(?:SHARED\s+)?RULESET:\s*(.+)$/im);
  const name = nameMatch?.[1]?.trim() || (metadata.id ? metadata.id.split('/').pop() : 'unnamed');

  // Use extends from frontmatter if available, otherwise parse from content
  const extends_ = metadata.extends.length > 0 ? metadata.extends : parseExtends(markdownContent);
  ...
  return {
    ...
    frontmatterWarnings: validation.warnings,
  };
}
...
function collectRulesetFiles(baseDir, cwd = process.cwd()) {
  ...
  function walk(currentDir) {
    fs.readdirSync(currentDir, { withFileTypes: true }).forEach((entry) => {
      ...
      if (entry.isFile() && entry.name.endsWith('.ruleset.md')) {
        files.push({
          fullPath,
...
function resolveLocalRuleset(name, cwd = process.cwd()) {
  ...
  const candidateName = `${name}.ruleset.md`;
  const locations = [
    ...collectRulesetFiles(path.join(cwd, '.grabby', 'rulesets'), cwd)
      .filter((entry) => path.basename(entry.fullPath).toLowerCase() === candidateName.toLowerCase())
      .map((entry) => entry.fullPath),
    ...collectRulesetFiles(path.join(cwd, 'docs', 'rulesets'), cwd)
      .filter((entry) => path.basename(entry.fullPath).toLowerCase() === candidateName.toLowerCase())
      .map((entry) => entry.fullPath),
...
function listRulesets(cwd = process.cwd()) {
  ...
  collectRulesetFiles(localDir, cwd).forEach((entry) => {
    const content = fs.readFileSync(entry.fullPath, 'utf8');
    const parsed = parseRuleset(content, entry.relativePath);
    rulesets.push({
      name: parsed.name,
  ...
  collectRulesetFiles(sharedDir, cwd).forEach((entry) => {
    const content = fs.readFileSync(entry.fullPath, 'utf8');
    const parsed = parseRuleset(content, entry.relativePath);
    rulesets.push({
      name: parsed.name,
