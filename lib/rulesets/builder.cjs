...
function discoverRulesetCandidates(baseDir) {
  const candidates = [
    'README.md',
    'docs',
    '.cursor/rules',
  ...
  return candidates
    ...
    .map((candidate) => path.relative(baseDir, candidate).replace(/\\/g, '/'));
}
...
async function runRulesetWizard(baseDir = process.cwd(), options = {}) {
  ...
  try {
    ...
    const suggestions = discoverRulesetCandidates(baseDir);
    if (suggestions.length > 0) {
      logger.log(`Suggested inputs: ${suggestions.join(', ')}`);
    }
    ...
    const suggestedCsv = suggestions.slice(0, 4).join(', ');
    const pathsCsv = options.pathsCsv || await askQuestion(rl, suggestedCsv ? `${pathPrompt}(suggested: ${suggestedCsv}) ` : pathPrompt);
...
/**
 * Create a ruleset in draft mode (returns content without writing)
 * @param {string} baseDir - Base directory
 * @param {object} options - Options
 * @param {string} options.goal - User's goal
 * @param {string} options.title - Ruleset title
 * @param {string} options.pathsCsv - Comma-separated paths for context
 * @param {string} options.mode - 'import-existing' or 'create-local'
 * @returns {Promise<{ content: string, metadata: object }>}
 */
async function createRulesetDraft(baseDir = process.cwd(), options = {}) {
  const mode = options.mode || 'create-local';
  const provider = options.provider || getAvailableProvider();
  const title = parseRulesetTitle(
    mode === 'import-existing' ? 'Imported Ruleset' : 'Local Repository Ruleset',
