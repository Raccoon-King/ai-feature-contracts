...
const { findLegacyTicketFiles } = require('./features.cjs');
const {
  resolveContextRefs,
...
`);
    ...
    const managedFiles = [
      { file: path.join(context.cwd, '.clinerules', '00-grabby-core.md'), content: '# Managed by Grabby — do not edit manually.\n\nCore governance rules are generated by Grabby.\n' },
      ...
      { file: path.join(context.cwd, '.continue', 'rules', '00-grabby-core.md'), content: '# Managed by Grabby — do not edit manually.\n\nCore governance rules are generated by Grabby.\n' },
    ];
    managedFiles.forEach(({ file, content }) => {
      if (file.includes('00-grabby-core')) {
    ...
    logger.log(`  ✓ Created ${path.relative(context.cwd, lockPath).replace(/\\/g, '/')}`);
  ...
  function validate(file) {
    ...
    const legacyTicketFiles = findLegacyTicketFiles(context.cwd);
  ...
  function audit(file) {
    ...
    logger.log(`\n${'─'.repeat(50)}`);
    if (!fs.existsSync(auditPath)) { fs.writeFileSync(auditPath, `# Audit: ${canonicalId}\n\n- Status: pending\n`); }
  ...
  function policyCheck() {
    ...
    try {
      ...
      logger.log(prefixedError('Unable to inspect git diff.', 'Run this command in CI with git available.'));
      exit(1);
      return;
    }

    const triggers = changed.length >= threshold || changed.some((f) => restricted.some((r) => f.startsWith(r)));
    if (!triggers) {
      logger.log('✓ Policy check passed');
      return;
    }

    const hasContract = fs.existsSync(context.contractsDir) && fs.readdirSync(context.contractsDir).some((f) => f.endsWith('.fc.md'));
    if (!hasContract) {
