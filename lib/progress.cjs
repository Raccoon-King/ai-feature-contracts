...
function createWorkflowTracker(steps, options = {}) {
  ...
  return {
    start: () => { printStatus(); spinner = createSpinner(steps[0], { color: 'cyan', stream }); spinner.start(); },
    next: (customText) => {
      if (spinner) spinner.succeed(customText || steps[currentStep]);
      completed.add(currentStep);
      currentStep++;
      if (currentStep < steps.length) { spinner = createSpinner(steps[currentStep], { color: 'cyan', stream }); spinner.start(); }
    },
