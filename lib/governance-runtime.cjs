...
function resolveContextRefs({ docsDir, contractContent, phase, tokenBudget }) {
  ...
  ['ARCH', 'RULESET', 'ENV'].forEach((kind) => {
    const ref = refs[kind] || index.defaults?.[kind];
    if (!ref) {
      return;
    }
    ...
    if (!section) {
      throw new Error(`Section not found for ${kind}:${ref} -> ${entry.section}`);
    }
...
function parsePlanFilePaths(planData) {
  return new Set((planData.files || []).map((item) => item.path));
}
...
function validateExecutionScope({ cwd, planData, contractContent }) {
  ...
  changed.forEach((file) => {
    ...
    const inRestricted = restricted.some((dir) => normalized.startsWith(dir.replace(/\*$/, '')));
    ...
    if (!existsInPlan) {
      const inAllowed = allowed.some((dir) => normalized.startsWith(dir));
      if (!inAllowed) violations.push(`New/modified file outside allowed directories: ${normalized}`);
    }
  });

  return { valid: violations.length === 0, violations, changedFiles: changed };
}
...
function saveFeatureMetrics(contractsDir, metrics) {
  const target = path.join(contractsDir, `${metrics.feature}.metrics.json`);
  fs.writeFileSync(target, `${JSON.stringify(metrics, null, 2)}\n`);
  return target;
}

function summarizeFeatureMetrics(contractsDir) {
  const files = fs.readdirSync(contractsDir).filter((f) => f.endsWith('.metrics.json'));
  const entries = files.map((file) => JSON.parse(fs.readFileSync(path.join(contractsDir, file), 'utf8')));
  return {
